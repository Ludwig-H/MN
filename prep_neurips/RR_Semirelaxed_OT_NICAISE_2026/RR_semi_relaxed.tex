...
\hyphenation{polonais problème couplage}

\newtheorem{demonstration}{Proof}
...
\title{Weekly report 5}
\author{Maxime Nicaise}
\newcommand{\applic}[4]{\ensuremath{\left|\begin{array}{ccl}#1 & \longrightarrow & #2 \\ #3 & \longmapsto & #4 \end{array}\right.}}
\newcommand{\applicc}[4]{%
  \ensuremath{%
    \left|
    \begin{array}{ccl}
      #1 & \longrightarrow & #2 \\[0.3em]
      #3 & \longmapsto & 
      \begin{array}[t]{l}
        #4
      \end{array}
...
\newcommand{\Hc}{\mathbf{H}(\operatorname{\bf curl} , \Omega)}
\newcommand{\Hco}{\mathbf{H}_0(\operatorname{\bf curl} , \Omega)}
\newcommand{\grad}{\operatorname{\bf grad}}
\newcommand{\Hd}{\mathbf{H}(\operatorname{div} , \Omega)}
\newcommand{\Hdo}{\mathbf{H}_0(\operatorname{div} , \Omega)}
...
\newcommand{\curl}{\operatorname{\bf curl}}
\newcommand{\R}{\mathbb{R}}
\newcommand{\capc}{\operatorname{Cap_{\curl}}}
\newcommand{\capcr}{\operatorname{Cap^{\overline{\Omega}}_{\curl}}}
\newcommand{\trT}{\operatorname{\bf Tr}_{\operatorname{T}}}
\newcommand{\trcurl}{\operatorname{\bf Tr}_{\curl}}
\newcommand{\trn}{\operatorname{Tr}_{n}}
\newcommand{\Hcurl}[1]{\mathbf{H}(\operatorname{\bm{curl}}, #1)}
\newcommand{\prop}[1]{(\mathcal{P}#1)}
\newcommand{\smoothtotheborder}[1]{\mathcal{D}(\overline{\Omega})^{#1}}
\newcommand{\smooth}[1]{\mathcal{D}(\Omega)^{#1}}
...
%\hyphenation{rela-tive, quel-conques, mo-dèle, données}
\hyphenation{rela-tive}
 \hyphenation{quel-conques}
 \hyphenation{ mo-dèle}
 \hyphenation{ données}
...
\authorhead{Nicaise\and Bermudez \and and Perlaza}
%% titre francais long
\RRtitle{Sur le transport optimal semi-relaxé}
%% English title
\RRetitle{On semi-relaxed optimal transport}
%%
\titlehead{On semi-relaxed optimal transport}
%%
\RRnote{Maxime Nicaise, Yaiza Bermudez and Samir M. Perlaza are with INRIA, Centre Inria d’Université Côte d’Azur, Sophia Antipolis 06902, France. 
Samir M. Perlaza is also with the GAATI Laboratory at the Université de la Polynésie Française, Faaa 98702, French Polynesia; and also with the Electrical and Computer Engineering Department at Princeton University, Princeton N.J. 08544, USA.
}
...
\RRabstract{}
%%
\RRmotcle{Optimisation, Transport Optimal, Transport Optimal Entropique, Transport Optimal Régularisé.}
\RRkeyword{Optimization, Optimal Transport, Entropic Optimal Transport, Regularized Optimal Transport.}
%%
\RRprojet{NEO}  % cas d'un seul projet
%%\RRprojets{Socrate}
%%
...
\textcolor{red}{l'intro est a refaire quasiment en entier pour bien introduire le transport optimal semi relaxé. A mon avis il faut aussi refondre toute l'organisation de ce document.}
\subsection{State of the art}
Intro sur le transport optimal semi relaxe. 
\subsection{Notations and preliminaries}
\textcolor{red}{Il y a des notations aajouter et d'autres à enlever.}
\textcolor{black}{Given a  function~$f: \mathcal{X} \to \mathcal{Y}$, its effective domain is denoted by  $\operatorname{dom}(f) \triangleq \{x \in \mathcal{X}: f(x) < +\infty\}$; and its range is denoted by $\operatorname{Ran}(f) \triangleq \left\lbrace f(x) : x \in \mathcal{X} \right\rbrace$. Given a set $\mathcal{A}$, its interior is denoted by $\operatorname{int}(\mathcal A)$. 
Given two measures $\mu$ and $\nu$, the notation $\mu \ll \nu$ signifies that $\mu$ is absolutely continuous with respect to $\nu$.} Across this report, $\mathcal{X}$ and $\mathcal{Y}$ are Polish spaces.
...
Let $\phi : \mathcal E \to \R \cup \{-\infty,+\infty \}$; the convex conjugate (or Fenchel conjugate) of $\phi$ is the function $\phi^* : \mathcal E' \to \R \cup \{-\infty,+\infty \}$ defined for $y \in \mathcal E'$ through (see \cite[Section~2.3]{convexanalysis})
\begin{IEEEeqnarray}{rCl}
...
\begin{definition}
  ...
  \end{IEEEeqnarray}
\end{definition}
\section{Semi-relaxed optimal transport}
Throughout this section, $(\mathcal{X},P)$ and $(\mathcal{Y},Q)$ are fixed probability spaces. The function $\mathsf{c}: \mathcal X \times \mathcal Y \to \R$ is a lower semi-continuous measurable function. Consider the problem of semi-relaxed optimal transport, introduced in , defined as follows
\begin{IEEEeqnarray}{rCl}
  \inf_{\pi \in \Pi(P,\star)} \int_{\mathcal X \times \mathcal Y} \mathsf{c}(x,y) \mathrm{d}\pi(x,y).
\end{IEEEeqnarray}
Relevant literature concerning this formulation includes ........... 
Standard approaches for solving semi-constrained optimal transport problems often rely on linear programming. However, these methods involve significant computational complexity, which frequently becomes prohibitive for large-scale datasets or high-dimensional problems.
\textcolor{red}{trouver une ref si possible pour le semi constrained qui traite le pb avec une solution de programmation lineaire. Expliquer que c'est pas bien d'un point de vue computationnel.}
\section{Sinkhorn-like regularization scheme}
Throughout this section, $(\mathcal{X},P)$ and $(\mathcal{Y},Q)$ are fixed probability spaces. The function $\mathsf{c}: \mathcal X \times \mathcal Y \to \R$ is a lower semi-continuous, measurable, bounded function, and $\alpha$ and $\beta$ are strictly positive numbers.
The objective is to extend the results established in \cite{cuturi2013sinkhorndistanceslightspeedcomputation} \textcolor{red}{quand meme etendre un peu la biblio associee a sinkhorn} to the framework of semi-relaxed optimal transport. This goal is achieved through a specific choice of regularization that ensures the dual problem is amenable to Sinkhorn-like iterations. The proposed scheme fits within the general framework introduced in \cite{chizat2018scaling}. More precisely, the problem is a particular instance of \cite[Equation 3.1]{chizat2018scaling} where $n=1$, the functional $\mathcal{F}_1$ is the indicator function of the set $\{P\}$, and $\mathcal{F}_2$ equals $\beta$ times the Kullback-Leibler (KL) divergence with respect to $Q$, with $\beta$ strictly positive. The reference measures, denoted as $dx$ and $dy$ in \cite{chizat2018scaling}, correspond here to $P$ and $Q$, respectively. This specialization leads to the following problem
\begin{IEEEeqnarray}{rCl}
...
\end{IEEEeqnarray}
\subsection{First dual point of view \textcolor{red}{changer le titre ou degager la section}}
...
\end{IEEEeqnarray}
\end{subequations}
\textcolor{red}{Dire qu'on peut faire quand meme un peu mieux surtout theoriquement que ces resultats qui ont ete etablis dans un cadre bien + general}

\subsection{Another dual formulation}
This section proposes an alternative dual formulation aiming to strengthen the convergence results of the potentials without requiring finite space assumptions. The derivation in \cite{chizat2018scaling} is highly general and does not explicitly account for the fact that the fasible couplings for the primal problems are necessarily probability measures. The alternative derivation presented here utilizes the variational formula of the KL divergence. The dual problem is subsequently obtained through the inversion of the infimum and supremum. This approach results in a formulation situated in the log-domain, which shares structural similarities with the notion of free energy in statistical physics \textcolor{red}{citer des refs pour cette idee}
\begin{IEEEeqnarray}{rCl}
  \nonumber
...
\begin{IEEEproof}
  ...
  \end{IEEEeqnarray}
  for all $t\in \R$ and $a>0$. Let $(x,y)$ be in $\mathcal X \times \mathcal Y$; applying the previous inequality with $a = \frac{\mathrm{d}\pi}{\mathrm{d}PQ}(x,y)$ and $t = u(x)+v(y)-\mathsf{c}(x,y)$ and integrating with respect to $PQ$ yields
  \begin{IEEEeqnarray}{rCl}
    \nonumber
...
\end{IEEEproof}
\textcolor{red}{faire la remqrque qu'en particulier cela implique que la valeur du dual est finie}
\subsubsection{Attainment}
\textcolor{red}{Dire que cette formulation duale, bien que très différente en apparence, donne une version à peine différente des itérations par maximisation alternée. La propriete suivante a l'air un peu inutile et en plus elle traduit le fait que la fonctionnelle $\mathsf D$ n'est pas strictement convexe, donc il n'y aura pas unicité des potentiels maximisants, mais en fait elle va etre utile pour prouver l'existence par contre de potentiels.}
\begin{lemme}
...
\end{IEEEproof}
\textcolor{red}{Ici il faut expliquer qu'on choisit la manière d'écrire "la differentielle vaut 0" mais en imposant une condition de normalisation, d'abord pour obtenir l'unicité, puis aussi parce que la solution du primal s'écrit plus simplement à partir de la solution particulière du dual qui vérifie ces 2 conditions de normalisation. Essayer de prouver un peu les 2 normalisations en question pour pouvoir y faire reference après.}
\begin{corollary}
...
\end{IEEEproof}
\textcolor{red}{la expliquer qu'on commence vraiment le travail qui va permettre d'arriver aux arguments de compacite du meme genre que ceux dans \cite{dimarino2020optimaltransportlossessinkhorn}, qui sont la cle de l'existence en fait}
\begin{lemme}
...
\end{lemme}
\begin{IEEEproof}
  The proof is divided in two parts, establishing \eqref{inegalite_osc_phi} and \eqref{inegalite_mon_dual_2} respectively. Let $v$ be a function in $\operatorname{L}^\infty(\mathcal{Y})$ and $(x,y)$ be in $\mathcal X  \times \mathcal Y$; the following inequalities are satisfied
\begin{IEEEeqnarray}{rCl}
  -\|\mathsf{c}\|_\infty \leq & -\mathsf{c}(x,y)&  \leq \|\mathsf{c}\|_\infty \\ 
   e^{\frac{-\|\mathsf{c}\|_\infty}{\alpha}}e^{\frac{v(y)}{\alpha}} \leq &  e^{\frac{v(y)-\mathsf{c}(x,y)}{\alpha}}  &\leq  e^{\frac{\|\mathsf{c}\|_\infty}{\alpha}}e^{\frac{v(y)}{\alpha}} \\ \label{ligne_de_calcul_qui_sert_dans_2_preuves}
    e^{\frac{-\|\mathsf{c}\|_\infty}{\alpha}}\int_{\mathcal Y}e^{\frac{v(y)}{\alpha}} \mathrm{d}Q(y)\leq &  \int_{\mathcal Y} e^{\frac{v(y)-\mathsf{c}(x,y)}{\alpha}}\mathrm{d}Q(y) &\leq   e^{\frac{\|\mathsf{c}\|_\infty}{\alpha}}\int_{\mathcal Y}e^{\frac{v(y)}{\alpha}}\mathrm{d}Q(y).
\end{IEEEeqnarray}
Taking the logarithm of the previous inequality and multiplying by $-\alpha$ yields
\begin{IEEEeqnarray}{rCl}
  \middlesqueezeequAggressive
  -\alpha \log\left(\int_{\mathcal Y}e^{\frac{v(y)}{\alpha}}\mathrm{d}Q(y)\right) - \|\mathsf{c}\|_\infty \leq \Phi(v)(x) \leq -\alpha \log\left(\int_{\mathcal Y}e^{\frac{v(y)}{\alpha}}\mathrm{d}Q(y)\right) + \|\mathsf{c}\|_\infty,
\end{IEEEeqnarray}
from which it can be deduced that $\operatorname{osc}(\Phi(v))\leq 2\|\mathsf{c}\|_\infty$, hence concluding the proof of \eqref{inegalite_osc_phi}.

To establish \eqref{inegalite_mon_dual_2}, let $\Psi_1$ and $\Psi_2$ denote the first and second terms of $\Psi$ in \eqref{formule_de_psi}, respectively. Consider a function $u \in \operatorname{L}^\infty(\mathcal{X})$. The oscillation of the constant part $\Psi_1(u)$ is zero, which implies the identity $\operatorname{osc}(\Psi(u)) = \operatorname{osc}(\Psi_2(u))$ . Apart from the probability measure $P$ and the multiplicative factor $\frac{\beta}{\alpha+\beta}$, the functional form of $\Psi_2$ is symmetric to the definition of $\Phi$ in \eqref{formule_de_phi} . The proof of \eqref{inegalite_osc_phi} establishes that $\operatorname{osc}(\Phi(v)) \leq 2\|\mathsf{c}\|_\infty$ . Adoption of an identical reasoning for $\Psi_2$ with its specific scaling leads to the following bound
  \begin{IEEEeqnarray}{rCl}
    \operatorname{osc}(\Psi(u)) \leq \frac{2\beta}{\alpha+\beta} \|\mathsf{c}\|_\infty,
  \end{IEEEeqnarray}
which concludes the proof of \eqref{inegalite_osc_psi}.
\end{IEEEproof}
\textcolor{red}{Maintenant qu on a un controle de l oscillation on se rapporche du controe en borne uniforme}
\begin{lemme}
...
\begin{IEEEproof}
  ...
  \begin{IEEEeqnarray}{rCl}
    -\frac{\|v\|_\infty+\|\mathsf{c}\|_\infty}{\alpha} \leq&\frac{v(y)-\mathsf{c}(x,y)}{\alpha} &\leq    \frac{\|v\|_\infty+\|\mathsf{c}\|_\infty}{\alpha} \\
    e^{-\frac{\|v\|_\infty+\|\mathsf{c}\|_\infty}{\alpha}} &\leq e^{\frac{v(y)-\mathsf{c}(x,y)}{\alpha}} \leq &    e^{\frac{\|v\|_\infty+\|\mathsf{c}\|_\infty}{\alpha}}.
  \end{IEEEeqnarray}
...
\end{IEEEproof}
\textcolor{red}{voila le lemme qui permet de borner uniformement une suite maximisante, et qui rend possible l utilisation de banach alaoglu pour l existence}
...
\begin{corollary}
  ...
  \begin{IEEEeqnarray}{rCl}
    \mathsf{D}(u,v)\leq \mathsf{D}(f,g), \text{ and } \|f\|_\infty,\|g\|_\infty \leq M\|\mathsf{c}\|_\infty,
  \end{IEEEeqnarray}
  where $\mathsf{D}$ is in \eqref{dual_maversion_fonctionnelle_C} and $M = \max(2,3\frac{\beta+1}{\alpha+\beta})$.
\end{corollary}
\textcolor{red}{juste ici il faut refaire la preuve. Ca change rien juste j'ai ecrit $\phi(u)$ donc faut remplacer u par v et je crois que quand on corrige cette confusion dans toute la suite ça fonctionne sans changer le résultat.}
\begin{IEEEproof}
  Let $(u,v)$ be a pair of functions in $\operatorname{L}^\infty(\mathcal X) \times \operatorname{L}^\infty(\mathcal{Y}) $. From Lemma \ref{lemme_osc_inegalites}, the function $\Phi(u)$ satisfies $\operatorname{osc}(\Phi(u))\leq 2 \|\mathsf{c}\|_\infty$, where $\Phi$ is in \eqref{formule_de_phi}. Thus, there exists a constant $A \in \R$ such that $g \triangleq \Phi(u)+A$ satisfies
  \begin{IEEEeqnarray}{rCl}
    \label{g__________________________}
    \|g\|_\infty\leq 2 \|\mathsf{c}\|_\infty.
  \end{IEEEeqnarray}
  ...
  \end{IEEEeqnarray}
  The combination of \eqref{g__________________________} and \eqref{f_________} ensures that $\|f\|_\infty, \|g\|_\infty \leq M\|\mathsf{c}\|_\infty$ with $M = \max(2, 3\frac{\beta+1}{\alpha+\beta})$. This results in a pair $(f,g)$ satisfying $\mathsf{D}(u,v) \leq \mathsf{D}(f,g)$, from \eqref{yczkduzedgzehdzehdez} and \eqref{fhgjkg}, which concludes the proof.
\end{IEEEproof}
\begin{theoreme}
...
\textcolor{red}{ici dire que parmi toutes les solutions au dual, on a fait exprès avec phi et psi de choisir celles qui vérifient naturellement des conditions de normalisation qui permettent d'écrire plus simplement la solution du probleme primal.}
\begin{theoreme}
...
\end{IEEEproof}
\textcolor{red}{faire une remarque que l'unicité du minimiseur primal permet en fait de justifier l'unicité du maximiseur dual qui vérifie simultanément \eqref{transform_pour_u} et \eqref{transform_pour_v}, et que c'est ce qu'on fait dans le corollaire suivant.}
\begin{corollary}
...
v_2(y)-v_1(y) &=& K.
\end{IEEEeqnarray}
Finally, combining \eqref{eq:v_diff_constant} with the normalization \eqref{normalisation_pour_psi}, which follows from \eqref{eq:pair1} and \eqref{eq:pair2}, yields
\begin{IEEEeqnarray}{rCl}
\label{eq:K_zero}
1
&=&
...
\end{IEEEproof}
\subsection{$(\alpha,\beta)$-approximation error control \textcolor{red}{renommer la section c naze}}
\textcolor{red}{Ici il faut faire l'intro, certainement citer les résultats de ceux qui ont travaillé sur des problèmes régularisés en OT en général auparavant. En gros on veut regarder comment le minimiseur du problème régularisé approxime le minimiseur du vrai problème quand $\alpha$ et $\beta$ varient. Et pareil pour la valeeur obtenue aussi. Dans cette partie il faut bien mettre l'accent sur un détail important: à cause de la présence de "l'a priori" $Q$, la 2e marginale de la solution du pb régularisé est contrainte à avoir le même support que $Q$. Donc déjà on ne pourra pas converger vers un minimiseur du vrai pb sauf s'il y en a un qui est absolument continu par rapport à $Q$. C'est quand même a bien garder en tête parce que ça donne une bonne idée de comment choisir $Q$.}
\subsection{Iterative algorithms}
\subsubsection{Presentation of the algorithms}
\textcolor{red}{la il faut faire une intro bien solide. Dire donc que comme le sinkhorn classique l'idée repose simplement sur de la maximisation alternée. Pourquoi on choisit les itérations avec $\Phi$ et $\Psi$ particulierement ? Parce que parmi les potentiels maximiseurs, $\Phi$ et $\Psi$ ont été construits pour donner ceux qui vérifient naturellement une contrainte de normalisation, pour que la mesure associée soit direct une proba sans avoir besoin de normaliser. Si on imposait pas ça, a chaque itération il y aurait des intégrales au dénominateur.}
\begin{algorithm}[ht]
  ...
  \end{subequations}
\end{algorithm}
\textcolor{red}{Clairement c'est a comparer avec la procédure proposée par Chizat dans \cite[Equation~3.6]{chizat2018scaling}. EN fait on voit que ce qu'on aurait obtenu c'est exactement la meme chose sauf que notre opérateur $\Psi$ a un terme en plus (la constante qui est le premier terme dans \eqref{formule_de_psi}), qui en plus est certainement chiant a calculer mais offre une meilleure stabiloté au moins sur le plan théorique (resultats de convergence bien meilleurs) et certainement aussi pour l'implémentation }
\begin{algorithm}[ht]
  \label{algo_2_scaled}
...
% \begin{lemme}
%   \label{lemme_operateurs_compacts}
%   Assume additionnally that $\mathcal X$ and $\mathcal{Y}$ are separable. Then, for all $1\leq p <+\infty$, the two following operators are continuous and compact
%   \begin{IEEEeqnarray}{rCl}
%     &&\Phi :\operatorname{L}^\infty(\mathcal Y) \to \operatorname{L}^p(\mathcal{X}); \\
%     &&\Psi :\operatorname{L}^\infty(\mathcal X) \to \operatorname{L}^p(\mathcal{Y}),
%   \end{IEEEeqnarray}
%   where $\Phi$ is defined in \eqref{formule_de_phi} and $\Psi$ is defined is in \eqref{formule_de_psi}.
% \end{lemme}
% \begin{IEEEproof}
%   Let $\Psi_1$ and $\Psi_2$ denote the first and second terms of $\Psi$ in \eqref{formule_de_psi}, respectively. Operator $\Phi$ is identical to the $(c,\epsilon,\Phi)$-transform $\mathcal{F}^{(c,\epsilon,\Phi)}$ introduced in \cite[Definition~3.1]{dimarino2020optimaltransportlossessinkhorn} for the specific instance where regularization is performed through the Kullback-Leibler divergence. Additionally, $\Psi_1$ is equal to a constant multiple of this same operator acting between the relevant functional spaces. The theoretical framework remains consistent with the reference; specifically, $\mathcal{X}$ and $\mathcal{Y}$ are complete separable spaces and the cost function $\mathsf{c}$ is bounded. Within this structure, $\alpha > 0$ serves as the regularization parameter $\epsilon$. Consequently, application of \cite[Proposition~D.1]{dimarino2020optimaltransportlossessinkhorn} ensures the continuity and compactness of both $\Phi$ and $\Psi_1$ in $\operatorname{L}^p$ spaces for $1 \leq p < +\infty$. Only the continuity and compactness of $\Psi_2$ remain to be proved to conclude the proof. Let $u_1,u_2$ be in $\operatorname{L}^\infty(\mathcal{X})$, let $x$ be in $\mathcal{X}$ and $y$. be in $\mathcal{Y}$. It holds that 
%   \begin{IEEEeqnarray}{rCl}
%     u_1(x) &\leq& u_2(x) + \|u_1-u_2\|_\infty\\
%     e^{\frac{u_1(x)-\mathsf{c}(x,y)}{\alpha}} &\leq& e^{\frac{\|u_1-u_2\|_\infty}{\alpha}}e^{\frac{u_2(x)-\mathsf{c}(x,y)}{\alpha}}.
%   \end{IEEEeqnarray}
%   Succesively integrating the previous inequality with respect to $P$, raising to the power $\frac{\alpha}{\alpha+\beta}$, integrating with respect to $Q$, taking the logarithm and multiplying by $\beta$ yields
%   \begin{IEEEeqnarray}{rCl}
%     \Psi_2(u_1) \leq \frac{\beta}{\alpha+\beta}\|u_1-u_2\|_\infty + \Psi_2(u_2).
%   \end{IEEEeqnarray}
%   And interchanging the funcctions $u_1$ and $u_2$ yields
%   \begin{IEEEeqnarray}{rCl}
%     |\Psi_2(u_1)-\Psi_2(u_2)| &\leq& \frac{\beta}{\alpha+\beta}\|u_1-u_2\|_\infty \\
%     \|\Psi_2(u_1)-\Psi_2(u_2)\|_{\operatorname{L}^p} &\leq& \frac{\beta}{\alpha+\beta}\|u_1-u_2\|_\infty.
%   \end{IEEEeqnarray}
%   The previous inequality yields the Lipschitz-continuity of $\Psi_2 :\operatorname{L}^\infty(\mathcal X) \to \operatorname{L}^p(\mathcal{Y}).$ Regarding its compactness, note that the range of $\Psi_2$ is contained in a finite-dimensional subspace of $\operatorname{L}^p(\mathcal{Y})$, that is the subspace of constant functions. Hence, \cite[Chapter~VI, Section~3]{conway}
%  implies that $\Psi_2 :\operatorname{L}^\infty(\mathcal X) \to \operatorname{L}^p(\mathcal{Y})$ is indeed compact, which concludes the proof. 
% \end{IEEEproof}
% \textcolor{red}{Essayer de justifier l'interet, soit pratique soit theorique de ce theoreme, c'est assez important a mon avis.}
% \begin{theoreme}
%   \label{theoreme_convergence_theorique_des_potentiels}
%   Assume additionnally that $\mathcal X$ and $\mathcal{Y}$ are separable. Let $(u^{(n)})$ and $(v^{(n)})$ be the iterates defined by algorithm \ref{algo_1_unscaled}, then for all $1\leq p<+\infty$, it holds that
%   \begin{IEEEeqnarray}{rCl}
%     u^{(n)} &&\xrightarrow[n\to +\infty]{\operatorname{L}^p(\mathcal X)} \hat{u}; \\
%     v^{(n)} &&\xrightarrow[n\to +\infty]{\operatorname{L}^p(\mathcal Y)} \hat{v}, 
%   \end{IEEEeqnarray}
%   where the pair $(\hat{u},\hat v) \in \operatorname{L}^\infty(\mathcal X) \times \operatorname{L}^\infty(\mathcal Y)$ solves problem \eqref{dual_maversion_fonctionnelle_C} and satisfies both \eqref{transform_pour_u} and \eqref{transform_pour_v}. In particular, it also holds that 
%   \begin{IEEEeqnarray}{rCl}
%     \label{convergence_theorique_valeurs_duales}
%     \mathsf{D}(u^{(n)},v^{(n)})\xrightarrow[n\to+\infty]{} \sup_{u\in \operatorname{L}^\infty(\mathcal{X}),v\in \operatorname{L}^\infty(\mathcal Y)} \mathsf{D}(u,v).   
%   \end{IEEEeqnarray}
% \end{theoreme}
% \begin{IEEEproof}
%   Let $(u^{(n)})$ and $(v^{(n)})$ be the iterates defined by Algorithm~\ref{algo_1_unscaled}.
%   The proof relies on two arguments; first it is proved that the sequences are precompact in every $\operatorname{L}^p$ for $1\leq p <+\infty$, and secondly it is proved that the set of $\operatorname{L}^p$-limit points is reduced to a singleton; precompactness in $\operatorname{L}^p$ (Lemma~\ref{lemme_operateurs_compacts}) then yields convergence of the whole sequence.First, note that, thanks to Corollary~\ref{lemme_transform}, for all $n\in \mathbb{N}$, it holds that
%   \begin{IEEEeqnarray}{rCl}
%     \label{suite_croissante_bornee}
%     \mathsf{D}(u^{(n)},v^{(n)})\leq \mathsf{D}(u^{(n+1)},v^{(n+1)}) \leq \sup_{u\in \operatorname{L}^\infty(\mathcal X), v\in\operatorname{L}^\infty(\mathcal Y)} \mathsf{D}(u,v)<+\infty,
%   \end{IEEEeqnarray}
%   where $\mathsf{D}$ is the dual functional in \eqref{dual_maversion_fonctionnelle_C}, and the finiteness follows from Lemma~\ref{lemme_dualite_faible}. Besides, from \eqref{inegalite_mon_dual_2}, the sequence $(v^{(n)})$ is uniformly bounded in $\operatorname{L}^\infty(\mathcal{Y})$. And from \eqref{inegalite_mon_dual_1}, $(u^{(n)})$ is thus uniformly bounded as well. Thus, from Lemma~\ref{lemme_operateurs_compacts} and the definition of the iterates in Algorithm \ref{algo_1_unscaled}, the two sequences are precompact in every $\operatorname{L}^p$ for $1\leq p<\infty$. Consider any limit points $\hat u \in \operatorname{L}^\infty(\mathcal X)$ and $\hat v \in \operatorname{L}^\infty(\mathcal Y)$ and associated subsequences $(u^{(n_k)})$, $(v^{(n_k)})$ such that 
%   \begin{IEEEeqnarray}{rCl}
%         u^{(n_k)} \xrightarrow[k\to +\infty]{} \hat u, && \text{in } \operatorname{L}^p(\mathcal X), \\
%         v^{(n_k)} \xrightarrow[k\to +\infty]{} \hat v, && \text{in } \operatorname{L}^p(\mathcal Y). 
%   \end{IEEEeqnarray}
%   From the continuity of the operators $\Phi$ in \eqref{formule_de_phi} and $\Psi$ in \eqref{formule_de_psi} in $\operatorname{L}^p$ (Lemma~\ref{lemme_operateurs_compacts}), it holds that 
%   \begin{IEEEeqnarray}{rCl}
%     \label{uuuuuu}
%         \Psi\!\left(u^{(n_k)}\right) = v^{(n_k+1)}  &\xrightarrow[k\to +\infty]{}& \Psi\!\left(\hat u\right), \qquad \text{in } \operatorname{L}^p(\mathcal Y), \\
%     \label{pobihbihb}
%         \Phi\!\left(v^{(n_k)}\right) = u^{(n_k+1)} &\xrightarrow[k\to +\infty]{}& \Phi\!\left(\hat v\right), \qquad \text{in } \operatorname{L}^p(\mathcal X). 
%   \end{IEEEeqnarray}
%   Besides, $\mathsf D$ is sequentially continuous on $\operatorname{L}^p$-convergent sequences that are uniformly bounded in $\operatorname{L}^\infty$: on bounded intervals, the maps $t\mapsto e^{t}$ and $t\mapsto e^{-t}$ are Lipschitz, hence the integral functionals in \eqref{dual_maversion_fonctionnelle_C} depend continuously on $(u,v)$ in $\operatorname{L}^p$, and the logarithm is continuous on $(0,+\infty)$. From \eqref{suite_croissante_bornee}, the real sequence $(\mathsf D(u^{(n)},v^{(n)}))$ is increasing and bounded, hence its increments vanish. In particular,
%   \begin{IEEEeqnarray}{rCl}
%   \label{egalite_dual_limite_1}
%   \mathsf D\!\left(\hat u,\Psi(\hat u)\right) - \mathsf D(\hat u,\hat v)
%   &=& \lim_{k\to +\infty}\Big[\mathsf D\!\left(u^{(n_k)},v^{(n_k+1)}\right)-\mathsf D\!\left(u^{(n_k)},v^{(n_k)}\right)\Big]
%   = 0,\\
%   \label{egalite_dual_limite_2}
%   \mathsf D\!\left(\Phi(\hat v),\hat v\right) - \mathsf D(\hat u,\hat v)
%   &=& \lim_{k\to +\infty}\Big[\mathsf D\!\left(u^{(n_k+1)},v^{(n_k)}\right)-\mathsf D\!\left(u^{(n_k)},v^{(n_k)}\right)\Big]
%   = 0.
%   \end{IEEEeqnarray}
%   Equality \eqref{egalite_dual_limite_1} and Corollary~\ref{lemme_transform} yield that $\hat v$ is a maximizer of $v\mapsto \mathsf D(\hat u,v)$.
%   Concavity and differentiability of $v\mapsto \mathsf D(\hat u,v)$ (Lemma~\ref{lemme_differentiation}) imply the first-order optimality condition \cite[Theorem~1, Chapter~7.4]{optimization_by_vector_space_methods}
%   \begin{IEEEeqnarray}{rCl}
%   \label{FOC_v}
%   \frac{\partial \mathsf D(\hat u,\hat v)}{\partial v} &=& 0.
%   \end{IEEEeqnarray}
%   Injecting \eqref{FOC_v} into \eqref{differentiele_de_C_par_rapport_a_v} yields the existence of a constant $C_v\in\mathbb R$ such that
%   \begin{IEEEeqnarray}{rCl}
%   \label{hatv_equal_psi_plus_const}
%   \hat v &\eqas{Q}& \Psi(\hat u) + C_v.
%   \end{IEEEeqnarray}
%   It remains to identify $C_v$. From the definition of $\Psi$ in \eqref{formule_de_psi}, the iterates satisfy the normalization
%   \begin{IEEEeqnarray}{rCl}
%   \label{normalisation_v_iteres}
%   \int_{\mathcal Y} \exp\!\left(-\frac{v^{(n_k)}(y)}{\beta}\right)\,\mathrm dQ(y) &=& 1 \qquad \text{for all }k.
%   \end{IEEEeqnarray}
%   Sequential continuity of $v\mapsto \int_{\mathcal Y} \exp(-v/\beta)\,\mathrm dQ$ along uniformly $\operatorname{L}^\infty$-bounded $\operatorname{L}^p$-convergent sequences implies
%   \begin{IEEEeqnarray}{rCl}
%   \label{normalisation_hatv}
%   \int_{\mathcal Y} \exp\!\left(-\frac{\hat v(y)}{\beta}\right)\,\mathrm dQ(y)
%   &=& \lim_{k\to+\infty}\int_{\mathcal Y} \exp\!\left(-\frac{v^{(n_k)}(y)}{\beta}\right)\,\mathrm dQ(y)
%   = 1.
%   \end{IEEEeqnarray}
%   The same normalization holds for $\Psi(\hat u)$, again from \eqref{formule_de_psi}:
%   \begin{IEEEeqnarray}{rCl}
%   \label{normalisation_psi_hatu}
%   \int_{\mathcal Y} \exp\!\left(-\frac{\Psi(\hat u)(y)}{\beta}\right)\,\mathrm dQ(y) &=& 1.
%   \end{IEEEeqnarray}
%   Combining \eqref{hatv_equal_psi_plus_const}, \eqref{normalisation_hatv}, and \eqref{normalisation_psi_hatu} yields
%   \begin{IEEEeqnarray}{rCl}
%   \label{Cv_zero}
%   1
%   &=& 
%   \int_{\mathcal Y}\exp\!\left(-\frac{\Psi(\hat u)(y)+C_v}{\beta}\right)\,\mathrm dQ(y) \\
%   &=&
%   e^{-C_v/\beta}\int_{\mathcal Y}\exp\!\left(-\frac{\Psi(\hat u)(y)}{\beta}\right)\,\mathrm dQ(y) \\
%   &=&
%   e^{-C_v/\beta},
%   \end{IEEEeqnarray}
%   hence $C_v=0$ and
%   \begin{IEEEeqnarray}{rCl}
%   \label{hatv_equal_psi_hatu}
%   \hat v &\eqas{Q}& \Psi(\hat u).
%   \end{IEEEeqnarray}
%   Equality \eqref{egalite_dual_limite_2} and Corollary~\ref{lemme_transform} yield that $\hat u$ is a maximizer of $u\mapsto \mathsf D(u,\hat v)$.
%   Concavity and differentiability of $u\mapsto \mathsf D(u,\hat v)$ (Lemma~\ref{lemme_differentiation}) imply the first-order optimality condition \cite[Theorem~1, Chapter~7.4]{optimization_by_vector_space_methods}
%   \begin{IEEEeqnarray}{rCl}
%   \label{FOC_u}
%   \frac{\partial \mathsf D(\hat u,\hat v)}{\partial u} &=& 0.
%   \end{IEEEeqnarray}
%   Injecting \eqref{FOC_u} into \eqref{differentiele_de_C_par_rapport_a_u} yields the existence of a constant $C_u\in\mathbb R$ such that
%   \begin{IEEEeqnarray}{rCl}
%   \label{hatu_equal_phi_plus_const}
%   \hat u &\eqas{P}& \Phi(\hat v) + C_u.
%   \end{IEEEeqnarray}
%   The definition of $\Phi$ in \eqref{formule_de_phi} implies the corresponding normalization for the iterates
%   \begin{IEEEeqnarray}{rCl}
%   \label{normalisation_u_iteres}
%   \int_{\mathcal X} \exp\!\left(\frac{u^{(n_k)}(x)}{\alpha}\right)\,\mathrm dP(x) &=& 1 \qquad \text{for all }k,
%   \end{IEEEeqnarray}
%   and sequential continuity of $u\mapsto \int_{\mathcal X}\exp(u/\alpha)\,\mathrm dP$ along uniformly $\operatorname{L}^\infty$-bounded $\operatorname{L}^p$-convergent sequences yields
%   \begin{IEEEeqnarray}{rCl}
%   \label{normalisation_hatu}
%   \int_{\mathcal X} \exp\!\left(\frac{\hat u(x)}{\alpha}\right)\,\mathrm dP(x)
%   &=& \lim_{k\to+\infty}\int_{\mathcal X} \exp\!\left(\frac{u^{(n_k)}(x)}{\alpha}\right)\,\mathrm dP(x)
%   = 1.
%   \end{IEEEeqnarray}
%   The same normalization holds for $\Phi(\hat v)$, again from \eqref{formule_de_phi}:
%   \begin{IEEEeqnarray}{rCl}
%   \label{normalisation_phi_hatv}
%   \int_{\mathcal X} \exp\!\left(\frac{\Phi(\hat v)(x)}{\alpha}\right)\,\mathrm dP(x) &=& 1.
%   \end{IEEEeqnarray}
%   Combining \eqref{hatu_equal_phi_plus_const}, \eqref{normalisation_hatu}, and \eqref{normalisation_phi_hatv} yields
%   \begin{IEEEeqnarray}{rCl}
%   \label{Cu_zero}
%   1
%   &=&
%   \int_{\mathcal X}\exp\!\left(\frac{\Phi(\hat v)(x)+C_u}{\alpha}\right)\,\mathrm dP(x) \\
%   &=&
%   e^{C_u/\alpha}\int_{\mathcal X}\exp\!\left(\frac{\Phi(\hat v)(x)}{\alpha}\right)\,\mathrm dP(x) \\
%   &=&
%   e^{C_u/\alpha},
%   \end{IEEEeqnarray}
%   hence $C_u=0$ and
%   \begin{IEEEeqnarray}{rCl}
%   \label{hatu_equal_phi_hatv}
%   \hat u &\eqas{P}& \Phi(\hat v).
%   \end{IEEEeqnarray}
%   Therefore $(\hat u,\hat v)$ satisfies simultaneously \eqref{transform_pour_u} and \eqref{transform_pour_v}. Uniqueness of such a pair (Corollary \ref{corollaire_unicite_des_potentiels_normalises}) implies that every $\operatorname{L}^p$-limit point coincides with $(\hat u,\hat v)$, hence the set of limit points is a singleton. Precompactness in $\operatorname{L}^p$ then yields convergence of the whole sequence towards $(\hat u,\hat v)$ in $\operatorname{L}^p(\mathcal X)\times \operatorname{L}^p(\mathcal Y)$. Finally, sequential continuity of $\mathsf D$ along uniformly $\operatorname{L}^\infty$-bounded $\operatorname{L}^p$-convergent sequences yields
% \begin{IEEEeqnarray}{rCl}
% \mathsf D(u^{(n)},v^{(n)}) \xrightarrow[n\to+\infty]{} \mathsf D(\hat u,\hat v),
% \end{IEEEeqnarray}
% and $\mathsf D(\hat u,\hat v)=\sup_{u\in \operatorname{L}^\infty(\mathcal{X}),v\in \operatorname{L}^\infty(\mathcal Y)}\mathsf D(u,v)$ since $(\hat u,\hat v)$ solves \eqref{dual_maversion_fonctionnelle_C}, which gives \eqref{convergence_theorique_valeurs_duales} and concludes the proof.
% \end{IEEEproof}
% \begin{corollary}
%   \label{corollaire_convergence_theorique_des_iteres_scaled}
%   Assume additionnally that $\mathcal X$ and $\mathcal{Y}$ are separable. Let $(a^{(n)})$ and $(b^{(n)})$ be the scaled iterates defined by algorithm \ref{algo_2_scaled}, then for all $1\leq p<+\infty$, it holds that
%   \begin{IEEEeqnarray}{rCl}
%     a^{(n)} &&\xrightarrow[n\to +\infty]{\operatorname{L}^p(\mathcal X)} e^{\frac{\hat{u}}{\alpha}}; \\
%     b^{(n)} &&\xrightarrow[n\to +\infty]{\operatorname{L}^p(\mathcal Y)} e^{\frac{\hat{v}}{\alpha}}, 
%   \end{IEEEeqnarray}
%   where the pair $(\hat{u},\hat v) \in \operatorname{L}^\infty(\mathcal X) \times \operatorname{L}^\infty(\mathcal Y)$ solves problem \eqref{dual_maversion_fonctionnelle_C} and satisfies both \eqref{transform_pour_u} and \eqref{transform_pour_v}. Moreover, it holds that
%   \begin{IEEEeqnarray}{rCl}
%     a^{(n)}\exp\left(-\frac{\mathsf{c}}{\alpha}\right)b^{(n)} = \exp\left(\frac{u^{(n)}\oplus v^{(n)}-\mathsf{c}}{\alpha}\right) &&\xrightarrow[n\to +\infty]{\operatorname{L}^p(\mathcal X \times \mathcal Y)} \frac{\mathrm{d}\hat{\pi}}{\mathrm{d}PQ}.
%   \end{IEEEeqnarray}
% \end{corollary}
% \begin{IEEEproof}
%   \textcolor{red}{a faire mais c'est facile, continuite de l'exponentielle tout est borné et c des mesures de proba.. }
% \end{IEEEproof}
...
\begin{theoreme}
  ...
  \begin{IEEEeqnarray}{rCl}
    ...
    &\leq&
    \tanh\!\left(\frac{\|\mathsf c\|_\infty}{\alpha}\right)\rho_{(\mathsf{c},\alpha,\beta)}^{\,l}\, d_{\mathrm H}\!\left(b^{(0)},\hat b\right),
  \end{IEEEeqnarray}
  ...
  \end{IEEEeqnarray}
\end{theoreme}
\textcolor{red}{la preuve suivante je l'ai bcp refaite donc a bien checker. Il faudra dire que c des trucs standards issus de méthodes pour les sinkhorn de base. Et aussi préciser un peu tout le vcabulaire technique sur les order preserving et homogenous of degree 1. Peut etre aussi pour les part of the cone meme si c connu normalement. Il manque aussi des from blablabla et des from blablabla pour les liens entre les inégalités.}
\begin{IEEEproof}
...
\textcolor{red}{A priori le + dur est fait maintenant l objectif est de deduire la convergence en objectif a une vitesse lineaire aussi, et pourquoi pas des convergences en norme uniforme a vitesse lineaire pour les potentiels (mais alors peut etre faut il se restreindre au cas ou X et Y sont finis, c'est sur que c deja traite dans la littérature pour le coup). Dans tous les cas aussi la convergence de la version "primale" des itérés doit etre traitée rapidement aussi. Après ça je ne vois que l implementation qui reste.}
\begin{corollary}
...
\end{corollary}
\begin{IEEEproof}
  The proof is split into three parts; the first one deals with \eqref{convergence_norme_infinie_linear_v}, the second one with \eqref{convergence_norme_infinie_linear_u}, and the third one with \eqref{convergence_norme_infinie_linear_primal}. Let $(u^{(l)},v^{(l)})$ be the unscaled iterates defined by Algorithm~\eqref{algo_1_unscaled}, $(a^{(l)},b^{(l)})$ be the scaled iterates defined by Algorithm~\eqref{algo_2_scaled}, and $(\hat u,\hat v)$ be the unique pair of maximizers satisfying \eqref{transform_pour_u} and \eqref{transform_pour_v} simultaneously. Recall that the two parametrizations are linked through
  \begin{IEEEeqnarray}{rCl}
  ...
  \begin{IEEEeqnarray}{rCl}
    \int_{\mathcal Y} \big(b^{(l)}(y)\big)^{-\alpha/\beta}\,dQ(y)=1,
    \qquad \mbox{and }
    \int_{\mathcal Y} \big(\hat b(y)\big)^{-\alpha/\beta}\,dQ(y)=1.
  ...
  \begin{IEEEeqnarray}{rCl}
    \int_{\mathcal Y} \big(b^{(l)}(y)\big)^{-\alpha/\beta}\,dQ(y)
    >
    \int_{\mathcal Y} \big(\hat b(y)\big)^{-\alpha/\beta}\,dQ(y),
  \end{IEEEeqnarray}
  ...
  \begin{IEEEeqnarray}{rCl}
    \middlesqueezeequAggressive
    e^{-\delta/\alpha}
    \!\!\int_{\mathcal Y}\!\exp\!\left(\frac{w(y)-\mathsf c(x,y)}{\alpha}\right)\,dQ(y)
    &\le& \middlesqueezeequAggressive
    \int_{\mathcal Y}\!\exp\!\left(\frac{v(y)-\mathsf c(x,y)}{\alpha}\right)\,dQ(y) \\
    &\le& \middlesqueezeequAggressive
    e^{\delta/\alpha}
    \!\!\int_{\mathcal Y}\!\exp\!\left(\frac{w(y)-\mathsf c(x,y)}{\alpha}\right)\,dQ(y). \ \ \ \ 
  \end{IEEEeqnarray}
...
\end{IEEEproof}
\textcolor{red}{Que reste-t-il à faire ? La convergence en objectif, la convergence des mesures associées par le primal sont des trucs qu'on va s'empresser d'obtenir. A FAIRE EN PRIORITE. On a toutes les clés en main faut juste le faire. Ensuite il reste l'immense partie expérimentale à traiter, chose qu'il faudra faire très sérieusement, en planifiant très très bien chaque étape avant de commencer, et surtout SANS s'éparpiller. Peut être même dans un autre rapport en fait ? En tout cas le plus gros questionnement reste le controle en $\alpha,\beta$ de l'erreur faite par rapport au vrai problème non régularisé. Peut-être qu'il faudrait dans un permier temps se contenter de la $\Gamma$ convergence qui peut apparemment être obtenue sans trop de gymnastique. Mais une borne satisfaisante sur cette erreur serait la cloture parfaite à tout ce dont on dispose...}
\textcolor{green}{J'ai fait ca c bon}

\begin{corollary}
  \label{corollaire_convergence_objectif}
  Let $(u^{(l)},v^{(l)})_{l\geq 0}$ be the iterates defined by
  Algorithm~\ref{algo_1_unscaled}. The sequence
  $\bigl(\mathsf{D}(u^{(l)},v^{(l)})\bigr)_{l\geq 0}$ is non-decreasing
  and converges to $\mathsf{P}(\hat\pi)$ at a linear speed:
  \begin{IEEEeqnarray}{rCl}
    ...
    d_{\mathrm{H}}\!\left(b^{(0)},\hat{b}\right),
  \end{IEEEeqnarray}
  where $\rho_{(\mathsf{c},\alpha,\beta)}$ is defined in \eqref{formule_rho}
  and $\hat{b}\triangleq e^{\hat{v}/\alpha}$.
\end{corollary}
\begin{IEEEproof}
  ...
  from which it cqn be deduced that
  \begin{IEEEeqnarray}{rCl}
...
\begin{corollary}
  ...
  Let $(u^{(l)},v^{(l)})_{l\geq 0}$ be the iterates defined by
  Algorithm~\ref{algo_1_unscaled}, and let
  $M\triangleq\max\!\bigl(2,\,3\tfrac{\beta+1}{\alpha+\beta}\bigr)$.
  For each $l\geq 0$, define the measure $\pi^{(l)}$ on
  $\mathcal{X}\times\mathcal{Y}$ through
  \begin{IEEEeqnarray}{rCl}
    ...
    \right).
  \end{IEEEeqnarray}
  Then $\pi^{(l)}\in\Pi(P,\star)$ for all $l\geq 0$, and the sequence
  $\left(\mathrm{d}\pi^{(l)}/\mathrm{d}PQ\right)$ converges to $\mathrm{d}\hat\pi/\mathrm{d}PQ$
  in $\operatorname{L}^\infty(\mathcal X \times \mathcal Y)$ at a linear speed, that is
  \begin{IEEEeqnarray}{rCl}
  ...
  \end{IEEEeqnarray}
  where $\rho_{(\mathsf{c},\alpha,\beta)}$ is defined in \eqref{formule_rho}.
  In particular, $\pi^{(l)}$ converges to $\hat\pi$ in total variation at
